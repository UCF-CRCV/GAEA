#!/usr/bin/env python3
"""
Script to create evaluation batches for country prediction accuracy.
...
def load_predictions(pred_path):
    ...
    for i in results:
        ...
        predictions[i['filename']] = i['predicted_answer'][0].split(',')[-1].strip()
    
    return predictions

def load_ground_truth(gt_path, benchmark, country_index):
    ...
    with open(gt_path, 'r') as f:
        ...
        for line in csvreader:
            if benchmark == 'yfcc4k':
                ...
                key = line[0]
            gts[key] = line[country_index]
    
    return gts

def get_country_index(benchmark):
    """Get the index of country information in the ground truth CSV."""
    if benchmark == 'yfcc4k':
        return 6
    elif benchmark in ['im2gps', 'im2gps3k', 'yfcc26k']:
        return 5
    elif benchmark in ['gws15k', 'geode']:
        return 4
    elif benchmark in ['cityguessr', 'dollarstreet']:
        return 2
    else:
...
def save_batch_file(batch, output_path, benchmark, model_folder, eval_type="country"):
...
def main():
    parser = argparse.ArgumentParser(description="Create evaluation batches for country predictions")
    ...
    for pred_path in pred_files:
        ...
        output_path = os.path.join(args.output_folder, f"{benchmark}_{Path(args.pred_folder).name}_country.jsonl")
        ...
        # Get country index based on benchmark
        country_index = get_country_index(benchmark)
        
        # Load predictions and ground truth
        predictions = load_predictions(pred_path)
        ground_truth = load_ground_truth(gt_path, benchmark, country_index)
